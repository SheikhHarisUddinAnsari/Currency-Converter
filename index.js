...
async function input() {
    const welcome = `Welcome to Money Exchange Teller`;
    ...
    const userCurrency = await inquirer
        ...
        .then((ans) => {
        const userCurrency = ans.userCurrency;
        return userCurrency;
    });
    const userAmount = await inquirer
        .prompt([
        {
            type: `input`,
            name: `userAmount`,
        ...
        .then((ans) => {
        const userAmount = ans.userAmount;
        return userAmount;
    });
    console.log(chalk.bgWhite(chalk.redBright(`You Entered ${userAmount} ${userCurrency}`)));
    const convCurrency = await inquirer
        ...
        .then((ans) => {
        const convCurrency = ans.convCurrency;
        return convCurrency;
    });
    switch (true) {
        case userCurrency == `US Dollar`:
            ...
            if (userCurrency == convCurrency) {
                console.log(chalk.bgWhite(chalk.redBright(`The amount will remain the same that is ${userAmount} ${userCurrency}`)));
            }
            break;
...
class US_Dollar {
    constructor(amount, user_currency, conv_currency) {
        let message;
        let amount2;
        switch (true) {
